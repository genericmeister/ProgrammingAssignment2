...
## The following function creates & returns the inverse of the matrix created thru 
...
cacheSolve <- function(x, ...) {
        ...
        m<-x$getinverse() ## Get the inverse stored in x
        if (!is.null(m)) 
        {
        ...
        matrix<-x$get() 
        m<-solve(matrix,...) ## solve is used to get the inverse of matrix
        x$setinverse(m) 
